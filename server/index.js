...
// POST route to store payment data
app.post('/api/store-details', async (req, res) => {
  const { role,name, email, phone, deliveryType, cart, totalWithGST } = req.body;

  const newUser = new User({ role,name, email, phone, deliveryType, cart, totalWithGST });
...
app.get('/api/get-details', async (req, res) => {
  try {
    const userDetails = await User.find(); // Fetch all user details
